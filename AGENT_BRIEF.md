...
```
> **`composite` / `dims` / `twHeat` / `stage` / `quadrant` 是跨期趨勢圖的唯一資料來源。**
...
1. **投顧知識庫目前只保留 3 天封存檔**（2026-08-03 觀察，v0.5 覆核仍是如此）。
   7 天窗口實際只會拿到 3–4 天的新聞側資料，共振判定的樣本會偏薄，
   **等於每期都會觸發第 2 節的樣本偏薄分支**——門檻在 v0.5 由 `< 3` 改為 `≤ 3`，
   因為剛好 3 天時舊寫法永遠不會觸發，而 3 天正是常態。
...
4. 跨期趨勢八格選得對不對，累積 4 期後檢視（連續組 3 格 ＋ v2 組 5 格）。
5. **監控庫 v2 的 `triggers` 尚未接入。** 它是已經定義好門檻的清單
   （例：`hy80` ＝ HY 利差 3 個月走闊 ≥80bp，附 `state`），
   正好是背離節「裁判方法」要的東西，也能直接拿來驗收上一期的 `watch`。
   v0.5 刻意不碰以控制改動量，下次優先。
