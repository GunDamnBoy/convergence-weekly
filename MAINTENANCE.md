...
## 7. 變更紀錄（自 `AGENT_BRIEF.md` 第 6 節移入，v0.7 起在此維護）

新的變更加在表格最上方。**為什麼改**跟**改了什麼**一樣重要。

| 日期 | 版本 | 改了什麼 | 為什麼 |
|---|---|---|---|
| 2026-08-08 | v0.7.1 | 瘦身後子代理獨立複查，修 12 處：**最重要是 `prepare.py` 的零新增判定 bug**——「（窗口內無檔）」在字串比較裡大於任何日期，導致 exit 3 永遠不會觸發；另修監控庫新舊判定改用 `meta.built`（history 盤後落後一格）、既有 clone 改為必 pull（殘留上週的 work/ 會靜靜拿舊資料）、時區統一台北、「一組要一起改」清單納入 `make_index.py` 與 `prepare.py`、brief「八件事」→九件事、§6 標題殘留、八格→九格、「表態禁語」補進 brief、「單邊只標記」補進 prompt、兩支腳本 docstring 的步驟編號中性化 | 第四次印證「改完要再比對一次」：這次抓到的 exit-3 bug 若上線，零新增保護等於不存在，而且它只在「某庫窗口內沒檔案」時才失效——正常測試永遠測不到。清單過期問題也是第三次發生（v0.4 三處、v0.5.1 四處、這次加兩支新腳本），規則改成「清單只在 brief 3.0 節末維護一份，其他地方引用」。 |
