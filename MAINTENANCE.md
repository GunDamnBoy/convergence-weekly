...
第 1 步：取資料
  ...
  ⚠️ 樣本不足的處置：若投顧側 ≤ 3 天或節目側 ≤ 2 天，必須在 about.run 註明樣本偏薄，
     且共振判定要保守。投顧庫目前只保留 3 天封存檔，這個分支幾乎每期都會踩到。
...
- [ ] 8/9 那期發布後，實際看一眼跨期趨勢。
      **第 002 期時應該只看到 3 格連續，v2 那 5 格一格都不會出現**——
      外殼要 ≥2 個點才畫線，而 v2 期別當時只有 1 期。這是正常的，不要當成故障。
      v2 五格要到第 003 期（8/16）才會出現。說明文字則從第 002 期起就該寫出「前 1 期為舊架構」。
- [ ] 監控庫 v2 的 `triggers` 尚未接入，下次優先（見第 6 節與 brief 待決事項第 5 條）
